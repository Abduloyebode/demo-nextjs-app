...
import { restoreDocument } from "@/app/dashboard/documents/actions";

export function DocumentTrashRow({ document }: { document: DocumentListItem }) {
    ...
    <li className="flex flex-col gap-3 rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-panel)] sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-xs text-[var(--muted)]">
          Deleted {document.deletedAt ? document.deletedAt.toLocaleDateString() : ""}
        </p>
      </div>
